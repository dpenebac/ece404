...
# python3 DES_image.py image.ppm key.txt image_enc.ppm
...
def encrypt(image_path, key_path, output_path):

    # Open files
    #plaintext_file = open(image_path)
    ...
    bv = BitVector(filename=image_path)
    ...
    #output_hex = output_bv.get_hex()

    FILEOUT = open(output_path, 'wb')

    # dont forget to write image header
...
def main():
    ...
    encrypt(args.image, args.key, args.output)
