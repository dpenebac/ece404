...
For this code I copied the SHA256 to make the SHA512 format. 
The main thing I changed are listed below

changed h0-h7 values based on lecture notes

changed k values based on lecture notes

changed howmanyzeros based on the padding length from 64 to 128 bits for length

changed word length to 80

changed step 2 shifting algo based on lecture notes

changed step 3 algo based on lecture notes

for every bitvector block changed the size from 32 to 64 as described in the lecture notes 
'''
...
class SHA512( object ):
    ...
    def sha256(self):
        ...
        length1 = bv1.length()
        howmanyzeros = ((1024 - 128) - length1) % 1024 # save for last bits
...
'''
python3 sha512.py input.txt output.txt
'''
...
if __name__ == '__main__':
    ...
    with open(sys.argv[2], 'w') as file_to_write:
        file_to_write.write(hashval_in_hex)
    
    #print(hashval_in_hex)